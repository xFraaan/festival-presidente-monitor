...
def _send_discord_message(webhook_url: str, screenshot: bytes) -> None:
    files = {"file": ("captura.png", screenshot, "image/png")}
    data = {"content": "🚨🚨🚨 Cambio Realizado!"}
...
def _send_discord_alert(webhook_url: str, screenshot: bytes) -> None:
    for i in range(1, 4):
        print(f"Sending Discord notification {i}/3 …")
        _send_discord_message(webhook_url, screenshot)
        if i < 3:
            time.sleep(2)
...
def main() -> None:
    ...
    try:
        html, screenshot = _fetch_page(TARGET_URL)
    ...
    try:
        _send_discord_alert(webhook_url, screenshot)
    except Exception as exc:
